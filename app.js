...
let _playAborted = false;
function stopAudio(){
  _playAborted = true;
  // stop the shared element that actually plays the dialogue
  if(_audioEl){
    try{ _audioEl.pause(); }catch(e){}
    try{ _audioEl.onended=null; _audioEl.onerror=null; }catch(e){}
    try{ _audioEl.currentTime=0; }catch(e){}
    try{ _audioEl.removeAttribute('src'); _audioEl.load(); }catch(e){}
  }
...
// One reusable <audio> element, unlocked on first user tap (iOS requirement).
let _audioEl = null;
...
// Must be called synchronously inside a tap handler (once) to unlock audio on iOS.
function unlockAudio(){
  if(_audioUnlocked) return;
  const a = getAudioEl();
  try{
    // a tiny silent play/pause on the shared element unlocks it for later programmatic plays
    a.muted = true;
    const p = a.play();
...
// Play one line on the shared element. Resolves with true if it actually played, false if it failed (missing file).
function lsPlayLine(dId, i, spk){
  return new Promise(resolve=>{
    let settled=false;
    const done=(ok)=>{ if(settled) return; settled=true; a.onended=null; a.onerror=null; resolve(ok); };
    const a = getAudioEl();
    // hard guard so we never hang forever, but generous so real audio isn't cut off
    const guard = setTimeout(()=>done(true), 20000);
    a.onended = ()=>{ clearTimeout(guard); done(true); };
    a.onerror = ()=>{ clearTimeout(guard); done(false); };
    try{
      a.src = audioPath(dId,i,spk);
      a.currentTime = 0;
      const p = a.play();
      if(p && p.catch){ p.catch(()=>{ clearTimeout(guard); done(false); }); }
    }catch(e){ clearTimeout(guard); done(false); }
  });
}
...
async function lsPlayDialogue(d){
  ...
  LS.playing=true;
  _playAborted=false;                  // fresh playback
  unlockAudio();                       // unlock within the tap gesture (iOS)
  const btn=$('#lsPlay'); const hint=$('#lsHint');
  const bubbles=[...document.querySelectorAll('.chat .bubble')];
  if(btn){ btn.style.pointerEvents='none'; btn.classList.add('playing'); }

  let anyPlayed=false, anyFailed=false;
  for(let i=0;i<d.lines.length;i++){
    if(_playAborted || LS.stage!=='dialogue' || state.tab!=='listening'){ break; }
    const ln=d.lines[i];
    ...
    const ok = await lsPlayLine(d.id, i, ln.spk);
    if(_playAborted){ break; }          // stopped mid-line
    if(ok) anyPlayed=true; else anyFailed=true;
    if(!ok && i===0){ break; }
  ...
  if(!anyPlayed && anyFailed){
    // no audio files present -> honest message, do NOT fake it
    if(hint) hint.textContent = state.lang==='en'?'AUDIO NOT LOADED':'АУДИО НЕ ЗАГРУЖЕНО';
    const nо=$('#lsNoAudio'); if(nо) nо.style.display='block';
  } else {
    if(hint) hint.textContent = state.lang==='en'?'PLAY AGAIN':'СЛУШАТЬ СНОВА';
  }
  // questions unlock after the first full listen (even if audio missing, so study isn't blocked)
