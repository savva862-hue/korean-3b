...
function go(tab){
  ...
  if(state.tab==='listening' && tab!=='listening'){ LS.stage='cover'; }
  if(state.tab==='reading' && tab!=='reading'){ RS.stage='cover'; }
  state.tab = tab;
...
async function lsPlayDialogue(d){
  ...
  stopAudio(); btn.style.pointerEvents='none'; btn.classList.add('playing');
  for(let i=0;i<d.lines.length;i++){
    ...
    await new Promise(res=>{
      const ln=d.lines[i];
      const a=new Audio(audioPath(d.id,i,ln.spk));
      _currentAudio=a;
      const done=()=>res();
      a.onended=done;
      a.onerror=()=>{
  ...
  bubbles.forEach(x=>x.classList.remove('active'));
  btn.style.pointerEvents=''; btn.classList.remove('playing');
...
function bindListening(){
  ...
  if(LS.stage==='dialogue'){
    ...
    const toQ=$('#lsToQ'); if(toQ) toQ.querySelector('.continue-btn').onclick=()=>{ stopAudio(); LS.stage='question'; LS.qidx=0; render(); window.scrollTo(0,0); };
  }
